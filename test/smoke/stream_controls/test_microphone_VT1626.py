...
def test_microphone(driver, logger):
    ...
    @log_step(logger, "Отключение микрофона")
    def turn_off_mic():
        base_page.click(wg_page.MICROPHONE_BUTTON)

    @log_step(logger, "Проверка значка кнопки MICROPHONE - состояние: ВЫКЛ.")
    ...
    @log_step(logger, "Проверка tooltip - текст: TURN ON MICROPHONE")
