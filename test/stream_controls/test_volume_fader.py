...
def test_volume_fader(driver, logger):
    ...
    @log_step(logger, "ШАГ 3. Нажатие кнопки MUTE")
    def turn_off_mute():
        base_page.click(wg_page.MUTE_BUTTON)

    @log_step(logger, "ШАГ 4. Проверка отображения Volume Fader - состояние: ВКЛ.")
    ...
    @log_step(logger, "ШАГ 8. Установка значений Volume Fader")
