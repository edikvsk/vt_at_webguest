...
def test_video_bitrate_12_5m(driver, logger):
    ...
    @log_step(logger, "Выбор Video Bitrate")
    ...
    @log_step(logger, "Проверка значения Video Bitrate в VT WebGuest Settings")
    def check_video_bitrate_field_value_vt():
        ...
        desktop_app_page.select_combobox_item_by_index(4, 8)
        desktop_app_page.click_button_by_name(DesktopAppPage.VT_OK_BUTTON)

    @log_step(logger, "Проверка значения поля Video Bitrate")
