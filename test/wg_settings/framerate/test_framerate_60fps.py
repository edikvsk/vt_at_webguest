...
def test_framerate_60fps(driver, logger):
    ...
    @log_step(logger, "Проверка значения Framerate в VT WebGuest Settings")
    def check_framerate_field_value_vt():
        ...
        desktop_app_page.select_combobox_item_by_index(1, 2)
        desktop_app_page.click_button_by_name(DesktopAppPage.VT_OK_BUTTON)

    @log_step(logger, "Проверка значения поля Framerate")
