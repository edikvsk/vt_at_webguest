...
@pytest.mark.parametrize("modified_url_fixture",
                         [f"{WEB_GUEST_PAGE_URL}?resolution=1280x720"],
                         indirect=True)
def test_url_resolution_1280_720(modified_url_fixture, driver, logger):
    @log_step(logger, "ШАГ 1. Проверка URL")
    def check_url(driver):
        expected_url = f"{WEB_GUEST_PAGE_URL}?resolution=1280x720"
