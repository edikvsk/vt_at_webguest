...
def test_url_with_selfie_f(driver, logger):
    ...
    def check_url(drv):
        drv.get(web_guest_url + "?selfie=f")
        expected_url = f"{web_guest_url}?selfie=f"
    ...
    @log_step(logger, "Проверка отображения окна Selfie - состояние: ВЫКЛ")
